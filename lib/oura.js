...
/**
 * Fetch daily activity summaries from Oura.
 * Returns steps, active calories, and activity score per day.
 *
 * @param {string} token - Personal Access Token
 * @param {string} startDate - YYYY-MM-DD
 * @param {string} endDate - YYYY-MM-DD
 * @returns {Promise<Array>}
 */
export async function getDailyActivity(token, startDate, endDate) {
  const params = new URLSearchParams({ start_date: startDate, end_date: endDate });
  const res = await fetch(`${API_BASE}/daily_activity?${params}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Oura daily_activity failed (${res.status}): ${text}`);
  }
  const json = await res.json();
  return json.data ?? [];
}
