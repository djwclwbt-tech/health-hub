...
export async function login(username, password) {
  ...
  // Step 2 — login with form encoding (CSRF check requires form, field name is "email" not "username")
  const postUrl = LOGIN_API_URL;
  const loginRes = await fetch(postUrl, {
    ...
    body: new URLSearchParams({ anticsrf, username, password }),
    redirect: 'manual',
  });
  ...
  if (!nonce) {
    ...
    throw new Error(`Cronometer: login failed (HTTP ${loginRes.status}). Response: ${body.slice(0, 200)}`);
  }
  ...
  const tokenMatch = tokenText.match(/OK\[\d+,\["([^"]+)"/) ?? tokenText.match(/OK\["([^"]+)"/);
  if (!tokenMatch) {
...
export function parseServings(csv) {
  ...
  const carbsCol    = col('Carbs (g)', 'Carbohydrates (g)', 'Net Carbs (g)');
  const fiberCol    = col('Fiber (g)');
  const categoryCol = col('Group', 'Category', 'Meal');
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const mealType = (row[categoryCol] || 'Other');
    ...
    // Flat format matching the app's meal item structure
    d.meals.push({ description: qty ? `${name} (${qty})` : name, cal, protein, carbs, fat, fiber, mealType });
  }
