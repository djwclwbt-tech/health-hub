...
export async function login(username, password) {
  // Step 1 — get anticsrf token from login page
  const pageRes = await fetch(LOGIN_PAGE_URL, { redirect: 'follow' });
  ...
  const pageCookies = getSetCookieHeaders(pageRes);
  ...
  if (!nonce) throw new Error('Cronometer: login failed — check CRONOMETER_USERNAME/PASSWORD');
